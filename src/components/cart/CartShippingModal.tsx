...
export default function CartShippingModal({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center bg-black/10 backdrop-blur-sm transition-all">
      <div className="bg-white rounded-t-2xl sm:rounded-2xl shadow-2xl w-full sm:w-full sm:max-w-lg lg:max-w-2xl mx-auto max-h-[95vh] sm:max-h-[90vh] min-h-[400px] min-w-[320px] sm:min-w-[400px] overflow-y-auto border border-gray-200">
        {/* Header - Sticky */}
        <div className="sticky top-0 bg-white border-b border-gray-100 p-4 sm:p-6 rounded-t-2xl sm:rounded-t-2xl shadow-sm z-10">
          <div className="flex items-center justify-between">
            <h3 className="text-lg sm:text-xl font-bold text-gray-900">Shipping Information</h3>
            <button
              onClick={onClose}
        ...
        <div className="p-4 sm:p-6">
          {isProcessingPayment ? (
            <div className="flex flex-col items-center justify-center py-12 sm:py-16">
              <div className="animate-spin rounded-full h-12 w-12 sm:h-16 sm:w-16 border-b-4 border-indigo-600 mb-4 sm:mb-6"></div>
              <p className="text-lg sm:text-xl font-bold text-gray-900 mb-2 text-center">Processing Payment...</p>
              <p className="text-sm sm:text-base text-gray-600 text-center px-4">Please wait while we redirect you to complete your payment.</p>
            </div>
          ) : (
            <form onSubmit={e => { e.preventDefault(); onSubmit(); }} className="space-y-4 sm:space-y-6">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
                <div>
                  ...
                  <input
                    ...
                    className={`w-full border border-gray-300 rounded-lg px-3 py-2.5 sm:px-4 sm:py-3 focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 text-sm sm:text-base placeholder:text-gray-400 text-gray-900 bg-gray-50 ${(isAuthenticated && typedUser?.first_name) ? 'bg-gray-100 text-gray-500 cursor-not-allowed opacity-70' : ''}`}
                ...
                <div>
                  ...
                  <input
                    ...
                    className={`w-full border border-gray-300 rounded-lg px-3 py-2.5 sm:px-4 sm:py-3 focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 text-sm sm:text-base placeholder:text-gray-400 text-gray-900 bg-gray-50 ${(isAuthenticated && typedUser?.last_name) ? 'bg-gray-100 text-gray-500 cursor-not-allowed opacity-70' : ''}`}
              ...
              <div>
                ...
                <input
                  ...
                  className={`w-full border border-gray-300 rounded-lg px-3 py-2.5 sm:px-4 sm:py-3 focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 text-sm sm:text-base placeholder:text-gray-400 text-gray-900 bg-gray-50 ${(isAuthenticated && typedUser?.email) ? 'bg-gray-100 text-gray-500 cursor-not-allowed opacity-70' : ''}`}
              ...
              <div>
                ...
                <input
                  ...
                  className={`w-full border border-gray-300 rounded-lg px-3 py-2.5 sm:px-4 sm:py-3 focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 text-sm sm:text-base placeholder:text-gray-400 text-gray-900 bg-gray-50 ${(isAuthenticated && typedUser?.phone) ? 'bg-gray-100 text-gray-500 cursor-not-allowed opacity-70' : ''}`}
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full border border-gray-300 rounded-lg px-3 py-2.5 sm:px-4 sm:py-3 focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 text-sm sm:text-base placeholder:text-gray-400 text-gray-900 bg-gray-50"
              ...
              <div className="mt-4 sm:mt-6 bg-gray-50 rounded-lg p-3 sm:p-4">
                <h4 className="text-base sm:text-lg font-semibold mb-3 text-gray-900">Cart Summary</h4>
                <div className="space-y-3 max-h-48 sm:max-h-64 overflow-y-auto">
                  {cartItems.map((item, idx) => (
                    <div key={item.productId || idx} className="bg-white rounded-lg p-3 border border-gray-200">
                      ...
                      <div className="flex items-center justify-between gap-2">
                        ...
                        <span className="text-sm font-medium text-indigo-600">
                          Total: ₦{((item.sellingPrice ?? item.price ?? 0) * item.quantity).toLocaleString()}
                        </span>
                      </div>
                    </div>
                  ))}
              ...
              {/* Action Buttons */}
              <div className="flex flex-col sm:flex-row gap-3 sm:gap-4 pt-4 sticky bottom-0 bg-white border-t border-gray-100 -mx-4 sm:-mx-6 px-4 sm:px-6 py-3 sm:py-4 pb-20 sm:pb-4">
                <Button
                  type="button"
                  variant="outline"
                  className="flex-1 border-gray-300 bg-white hover:bg-gray-100 text-gray-700 font-semibold shadow-sm py-2.5 sm:py-3"
                  onClick={onClose}
                >
                  Cancel
                </Button>
                <Button
                  ...
                  className="flex-1 bg-gradient-to-r from-indigo-600 to-purple-600 hover:from-indigo-700 hover:to-purple-700 text-white font-bold py-2.5 sm:py-3 rounded-lg shadow-lg disabled:opacity-50 disabled:cursor-not-allowed text-sm sm:text-base"
                >
