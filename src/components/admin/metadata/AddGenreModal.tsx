...
export default function AddGenreModal({ open, onClose, onSuccess }: AddGenreModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setError(null);
    try {
      const res = await api.admin.addGenre(name, description);
      if (res.success) {
        setName("");
        setDescription("");
        onSuccess();
      } else {
        setError(res.message || "Failed to add genre");
      }
    } catch {
